import { RemoteProfileService } from "../../../services/RemoteProfileService.ts";
 ...
 * RemoteProfileService directly (the same collaborator ExistingWPStrategy's
 ...
 *
 * `remoteProfileServiceFactory` is injectable (defaulting to the real
...
export function createProfileImportSource(
  id: string,
  label: string,
  remoteProfileServiceFactory: RemoteProfileServiceFactory = (profile) => new RemoteProfileService(profile),
): ImportSource {
  const remote = (ctx: ImportSourceContext) => remoteProfileServiceFactory((ctx as ProfileImportContext).profile);

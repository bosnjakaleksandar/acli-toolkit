...
export function registerPresetCommand(program) {
  ...
  command.command("list").option("--config <path>").option("--json", "Output machine-readable JSON").action(async (options) => {
  ...
  command.command("inspect <name>").option("--config <path>").action(async (name, options) => {
    const { config } = await loadConfig({ configPath: options.config, resolveSecrets: false });

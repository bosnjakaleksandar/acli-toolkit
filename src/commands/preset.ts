...
export function registerPresetCommand(program: Command): void {
  ...
  command.command("list").option("--config <path>").option("--json", "Output machine-readable JSON").action(async (options: any) => {
  ...
  command.command("inspect <name>").option("--config <path>").action(async (name: string, options: any) => {
    const { config } = await loadConfig({ configPath: options.config, resolveSecrets: false });

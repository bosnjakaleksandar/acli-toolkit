...
export default class ReactStrategy extends ScaffoldStrategy {
  run: Runner;
  ...
  override async scaffold(targetDir: string, ctx: any, spinner: Spinner | null = null): Promise<void> {
    ...
    // Keep Vite's own .gitignore and add A-CLI's shared rules (env files,
    // editors, OS files, .acli/) that it doesn't cover.
    await applyGitignoreTemplate(targetDir, "react");
    // Docker/Lando dev server, unless the project runs natively.
    await this.envService?.scaffold(targetDir, "react", ctx, spinner);
  }
}

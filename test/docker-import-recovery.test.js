...
test("regression: recovers when the raw SQL import succeeds (root creds) but WordPress's own DB connection fails (stale app-user creds in an old volume)", async () => {
  ...
  const isWpCliInstallCommand = (args) => args.join(" ").includes("mv wp-cli.phar /usr/local/bin/wp");
  ...
  const installCalls = calls.filter((call) => call.includes("mv wp-cli.phar /usr/local/bin/wp"));
...
test("regression: recovery removes a leftover wp-config.php, since down --volumes never touches the bind-mounted project directory", async () => {
  ...
  assert.equal(await fs.pathExists(path.join(directory, "wp-config.php")), false, "recovery must remove the stale wp-config.php so the entrypoint regenerates it");
...
test("recoverDb probes the app-path DB connection after restarting the stack, before reinstalling wp-cli", async () => {
  ...
  const installIndex = calls.findIndex((call) => call.includes("mv wp-cli.phar /usr/local/bin/wp"));
